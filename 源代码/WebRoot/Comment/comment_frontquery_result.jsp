		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#commentListPanel" aria-controls="commentListPanel" role="tab" data-toggle="tab">博客评论列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>Comment/comment_frontAdd.jsp" style="display:none;">添加博客评论</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>回复id</td><td>被评博客</td><td>评论内容</td><td>评论人</td><td>评论时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<commentList.size();i++) {
					    	            		...
					    	            		Comment comment = commentList.get(i); //获取到博客评论对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=comment.getCommentId() %></td>
 											<td><%=comment.getBlogObj().getTitle() %></td>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="commentQueryForm" id="commentQueryForm" action="<%=basePath %>Comment/frontlist" class="mar_t15" method="post">
            <div class="form-group">
            	<label for="blogObj_blogId">被评博客：</label>
                <select id="blogObj_blogId" name="blogObj.blogId" class="form-control">
                	<option value="0">不限制</option>
	 				<%
	 				for(Blog blogTemp:blogList) {
	 					String selected = "";
 					if(blogObj!=null && blogObj.getBlogId()!=null && blogObj.getBlogId().intValue()==blogTemp.getBlogId().intValue())
 						selected = "selected";
	 				%>
 				 <option value="<%=blogTemp.getBlogId() %>" <%=selected %>><%=blogTemp.getTitle() %></option>
	 				<%
	 				}
	 				%>
 			</select>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;博客评论信息编辑</h4>
      </div>
